...
3. `model` is seeded into the coagent's `initialState.model` (`Main.tsx:13`,
   `ResearchCanvas.tsx`), so it travels to the backend as `state.model`.
